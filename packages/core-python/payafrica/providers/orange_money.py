...
class OrangeMoneyProvider(PaymentProvider):
    ...
    def __init__(self, client: httpx.AsyncClient, client_id: str, client_secret: str, merchant_code: str, sitename: str, callback_url: str, webhook_api_key: str, environment: str = "sandbox") -> None:
    ...
    async def check_status(self, session_id: str) -> PaymentStatus:
        ...
        status = transactions[0].get("status") if isinstance(transactions, list) and transactions and isinstance(transactions[0], dict) else None
    ...
    async def handle_webhook(self, raw_body: str | bytes, headers: WebhookHeaders) -> PaymentEvent:
        ...
        return PaymentEvent(id=str(payload.get("id", session_id)), session_id=session_id, status=self._status(status), occurred_at=str(payload.get("timestamp", "1970-01-01T00:00:00Z")), reference=payload.get("reference") if isinstance(payload.get("reference"), str) else None)
    ...
    def _response(self, response: httpx.Response) -> dict[str, Any]:
        ...
        if response.is_error:
            code = str(payload.get("code", response.status_code)); error = {"2020": PaymentError.INSUFFICIENT_FUNDS, "2021": PaymentError.INSUFFICIENT_FUNDS, "2000": PaymentError.INVALID_PHONE, "2001": PaymentError.INVALID_PHONE, "500": PaymentError.PROVIDER_TIMEOUT, "50": PaymentError.PROVIDER_TIMEOUT, "51": PaymentError.PROVIDER_TIMEOUT, "1": PaymentError.PROVIDER_TIMEOUT, "2": PaymentError.PROVIDER_TIMEOUT, "5": PaymentError.PROVIDER_TIMEOUT}.get(code, PaymentError.UNKNOWN)
    ...
    def _status(self, status: object) -> PaymentStatus:
        return {"ACCEPTED": PaymentStatus.SUCCESS, "SUCCESS": PaymentStatus.SUCCESS, "PENDING": PaymentStatus.PENDING, "INITIATED": PaymentStatus.PENDING, "CANCELLED": PaymentStatus.FAILED, "REJECTED": PaymentStatus.FAILED, "FAILED": PaymentStatus.FAILED}.get(str(status).upper(), PaymentStatus.FAILED)
